...
# Inclination tilts the orbit out of the x-y plane by mixing vy and vz
def inclined_velocity(v_mag, inclination_deg):
    inc = np.radians(inclination_deg)
    return [0, v_mag * np.cos(inc), v_mag * np.sin(inc)]
 
# Initial states: [x, y, z, vx, vy, vz]
# Each satellite starts at (r0, 0, 0) but with a different orbital inclination
sat1 = [r0, 0, 0] + inclined_velocity(0.8 * v_c, 0)    # elliptical,   0° inclination
sat2 = [r0, 0, 0] + inclined_velocity(v_c, 45)   # circular,    45° inclination
sat3 = [r0, 0, 0] + inclined_velocity(v_e, 90)   # parabolic,   90° inclination
sat4 = [r0, 0, 0] + inclined_velocity(1.2 * v_e, 60)   # hyperbolic,  90° inclination (polar)
...
for i in range(n):
    ...
    r_min = np.min(r)
    r_max = np.max(r)
    print(f"Satellite {i+1} ({orbit_types[i]}, inclination {[0,30,60,90][i]}°):")
